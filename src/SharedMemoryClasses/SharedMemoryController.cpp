...
void SharedMemoryController::addPacket(const u_char *text,const char *timeRecv){
    boost::mutex mtx;
    boost::mutex::scoped_lock lock(mtx);
    m_counter++;
    m_vector.push_back(text,timeRecv,m_typeOfInterface);
...
void SharedMemoryController::clearPackets(){
    m_vector.clear();
}
